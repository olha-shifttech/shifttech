...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> { const { slug } = await params; const item = entitiesByKind("transmissions").find((x) => x.slug === slug); return item ? { title: `${item.name} transmission`, description: item.description, alternates: { canonical: entityUrl(item) } } : {}; }
